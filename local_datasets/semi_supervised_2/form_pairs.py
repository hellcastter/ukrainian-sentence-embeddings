"""
Run: python3 -m local_datasets.semi_supervised_2.form_pairs
"""
...
DATASET_PATH = "local_datasets/semi_supervised/merged_collected_and_generated_2.json"
OUTPUT_CSV = "local_datasets/semi_supervised_2/pairs_semi_supervised_2.csv"
    ...
    with open(OUTPUT_CSV, "w", encoding="utf-8", newline="") as csvfile:
        ...
        for lemma, meanings in tqdm(data.items(), total=len(data)):
            for meaning_idx, meaning_data in enumerate(meanings.values()):
                positives = list(set(meaning_data["meaning"]["gloss"]))
                ...
                for index, sentence in enumerate(sentences):
                    ...
                    max_sentences = min(
                        recommended_sentences_with_anchor, len(positives) * len(anchors)
                    )
                    ...
                    for _ in range(max_sentences):
                        ...
                        while (anchor, positive) in used:
                            positive = random.choice(positives)
                            anchor = random.choice(anchors)

                        used.add((anchor, positive))
