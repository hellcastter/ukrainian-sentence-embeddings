...
from services.config import (
    MINIMUM_POS_OCCURRENCE,
    MINIMUM_GLOSS_OCCURRENCE,
    FREQUENCY_QUANTILES,
)


def prediction_accuracy(data_with_predictions):
    data_dropna = data_with_predictions.copy().dropna()
    data_dropna["gloss"] = data_dropna["gloss"].apply(lambda x: x[0])
    data_dropna["predicted_context"] = data_dropna["predicted_context"].apply(
        lambda x: x[0]
    )
...
def generate_results_by_pos(data_with_predictions, udpipe_model):
    ...
    results = {
        "overall_accuracy": [
            prediction_accuracy(data_with_predictions),
    ...
    for pos in data_with_predictions["pos"].value_counts().items():
        if pos[1] < MINIMUM_POS_OCCURRENCE:
            break
        data_with_predictions_pos = data_with_predictions[
            data_with_predictions["pos"] == pos[0]
        ]
        results[pos[0] + "_accuracy"] = [
            prediction_accuracy(data_with_predictions_pos),
            len(data_with_predictions_pos),
        ]
    results_df = pd.DataFrame.from_dict(
        results, orient="index", columns=["accuracy", "count"]
    )
    return results_df
...
def generate_results_by_count_of_gloss(data_with_predictions):
    ...
    data_with_predictions["gloss_count"] = data_with_predictions["lemma"].map(
        gloss_count
    )
    ...
    for gloss_count in (
        data_with_predictions["gloss_count"].value_counts().sort_index().items()
    ):
        if gloss_count[1] < MINIMUM_GLOSS_OCCURRENCE:
            data_with_predictions_gloss = data_with_predictions[
                data_with_predictions["gloss_count"] >= gloss_count[0]
            ]
            results[f"word_that_have_{gloss_count[0]}+_glosses"] = [
                prediction_accuracy(data_with_predictions_gloss),
...
def results_by_lemma_order(data_with_predictions):
    data_with_predictions["lemma_order"] = (
        data_with_predictions.groupby("lemma").cumcount() + 1
    )
...
def generate_results_filter_gloss_frequency(data_with_predictions):
    ...
    for i in range(1, max_gloss_for_lemma + 1):
        data_with_predictions_filtered_gloss = take_first_n_glosses(
            data_with_predictions, i
        )
        results[i] = [
            prediction_accuracy(data_with_predictions_filtered_gloss),
            len(data_with_predictions_filtered_gloss),
        ]
    results_df = pd.DataFrame.from_dict(
        results, orient="index", columns=["cum_accuracy", "cum_count"]
    )
...
def generate_results_filter_lemma_frequency(data_with_predictions, udpipe_model):
    ...
    if "freq_in_corpus" not in data_with_predictions.columns:
        data_with_predictions = add_frequency_column(
            data_with_predictions, udpipe_model
        )

    quantiles = [
        round(i * 1 / FREQUENCY_QUANTILES, 1) for i in range(1, FREQUENCY_QUANTILES)
    ]
...
def results_reports(
    ...
    if frequency_report:
        frequency_results = generate_results_filter_lemma_frequency(
            data_with_predictions, udpipe_model
        )
...
def prediction_comparison(base, new, udpipe_model):
    ...
    print(
        f"Improved by {round((accuracy_new - accuracy_base) * 100, 3)}%, its {round((accuracy_new - accuracy_base) * base.shape[0], 0)} samples"
    )
    correct_base_miss_new = base[
        (base.gloss == base.predicted_context) & (new.gloss != new.predicted_context)
    ]
    correct_new_miss_base = new[
        (base.gloss != base.predicted_context) & (new.gloss == new.predicted_context)
    ]
    ...
    results_base = results_reports(
        base, udpipe_model, verbose=False, return_results=True
    )
